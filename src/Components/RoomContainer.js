...
const RoomContainer = ({ data ,fromDate,toDate}) => {
  ...
  return (
    <Row className="justify-content-center mt-3">
      <Col xs={12} md={10} xl={5} style={{ ...columnStyle, marginRight: '5px' }}>
      ...
      <Col xs={12} md={10} lg={3} style={{ ...columnStyle, marginLeft: '5px' }}>
        <div className="d-flex flex-column">
          ...
          <div>
            ...
            <Link to={`./book/${data._id}/${fromDate?fromDate:"unknown"}/${toDate?toDate:"unknown"}`}>
           {
            fromDate ==''&& toDate==''? '':<button className='btn btn-success'>Book Now</button>
           }
          
          
            </Link>
            
            <button className='btn btn-success m-2'onClick={handleShow}>View Details</button>
          </div>
        </div>
      </Col>
