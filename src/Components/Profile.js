...
const MyBookings = () => {
  ...
  // https://backendhotel-1.onrender.com/api
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post(
          "https://hotel--backend.up.railway.app/api/getBookingByUserId",
          { userId: user._id },
        );
  ...
  const handleCancelBooking = async (roomId) => {
    try {
      ...
      await axios.post("https://hotel--backend.up.railway.app/api/cancelBooking", { roomId });
            ...
            <Meta
              ...
              description={
                <span style={{ color: "#666" }}>
                  <b>Status</b>: {room.status ? "Confirmed" : "canceled"}
                </span>
              }
            />
            <Space direction="vertical" style={{ marginTop: 16 }}>
              <p style={{ color: "#333" }}>
                <b>Transaction ID</b>: {room.transactionId}
              </p>
              <p style={{ color: "#333" }}>
                <b>Total Amount</b>: ${room.totalAmount}
              </p>
              <p style={{ color: "#333" }}>
                <b>Total Days</b>: {room.totalDays}
              </p>
              <p style={{ color: "#333" }}>
